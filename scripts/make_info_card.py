...
colored key/value rows for role, education, tech stack, and focus areas.
...
ROWS = [
    ("host",),
    ("kv", "Role", "B.Tech CSE Student"),
    ("kv", "Edu", "Techno India University, WB"),
    ("kv", "Focus", "GSoC 2027 · Security Research"),
    ("kv", "GitHub", "Roshan-Mallick"),
    ("gap",),
    ("sec", "Stack"),
    ...
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Building systems, learning deeply"),
    ("bul", "Solving real problems through code"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">roshan@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">roshan</tspan><tspan fill="{MUTED}">@</tspan>'
